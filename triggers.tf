...
resource "aws_s3_bucket_notification" "bucket_notification" {
  ...
  lambda_function {
    lambda_function_arn = aws_lambda_function.scanLambda.arn
...
resource "aws_lambda_permission" "s3InvokePermission" {
    ...
    function_name   = aws_lambda_function.scanLambda.function_name
...
resource "aws_cloudwatch_event_target" "av_update" {
  ...
  arn       = aws_lambda_function.dbUpdateLambda.arn
}
...
resource "aws_lambda_permission" "allow-cloudwatch-execution" {
  ...
  action              = "lambda:InvokeFunction"
  function_name       = aws_lambda_function.dbUpdateLambda.function_name
